...
// box — `slope` = fraction of the height dropped at the top's +Z edge; `curve`
// bends that sloped top (-1 concave .. +1 convex).
export function box(w = 1, h = 1, d = 1, { slope = 0, curve = 0 } = {}) {
  const s = q4(Math.max(0, Math.min(slope, 0.99)));
  ...
  const gen = () => {
    ...
    for (let i = 0; i <= N; i++) {                           // top, +z -> -z
      const u = 1 - i / N;
      o.push([-0.5 + u, 0.5 - s * (u - k * u * (1 - u)), N > 1 && i > 0 && i < N]);
    }
    return extrudeX(o, 1);
  };
