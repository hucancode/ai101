...
for (const path of ["./demo-shapes.js", "./demo-joints.js", "./engines/modeling.js", "./engines/joint.js"]) {
  try {
...
function select(subject) {
  ...
  // to make the sliders track the motion.
  ...
  const rows = built.channels.map((c) => {
    ...
    // pose live — so the motion departs from where the hand left it.
    let held = false;
    input.addEventListener("pointerdown", () => {
      held = true;
      choreo.reclaim(c.key);
    });
