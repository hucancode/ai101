...
// the catalog: a builder per entry, its default modeling params, and a
// standalone preview. A joint's mount slots come off jointMounts().
const JOINT_BUILDERS = { hinge1, hinge2, ball1 };
export const JOINT_NAMES = Object.keys(JOINT_BUILDERS);
export function jointModel(name, seed = 1, p = null, pose = null) {
  return collect(JOINT_BUILDERS[name], seed, { ...JOINT_PARAMS[name], ...(p || {}) }, pose);
}
