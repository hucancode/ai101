...
import { JOINT_KIT } from "./joints.js";

// The whole joint catalog in a row, each mechanism articulating on its own
// pose axes. The rotary joints take DEGREES; prismatic1 is the one LINEAR
// joint, so its pose is a travel distance in model units.
const SEED = 1;
const SPACING = 2.4;
...
const POSES = {
  ...
  pivot1: (t) => ({ spinA: 90 * Math.sin(t * 0.9), spinB: -90 * Math.sin(t * 0.9) }),
  prismatic1: (t) => ({ slideA: 0.09 * (1 + Math.sin(t)), slideB: 0.09 * (1 + Math.sin(t * 1.3)) }),
...
viewer.onFrame = (t) => {
  const items = [];
  JOINT_KIT.names.forEach((name, i) => {
    const { items: its } = JOINT_KIT.partModel(name, SEED, PARAMS[name] ?? null, POSES[name](t));
    const x = (i - (JOINT_KIT.names.length - 1) / 2) * SPACING;
