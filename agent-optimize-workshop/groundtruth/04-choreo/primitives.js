...
// PRIMITIVE ENGINE. A primitive call returns a HANDLE { key, id, mesh, m, t }:
// a reference to a shared UNIT MESH plus an affine transform (3x3 linear,
// row-major, + translation) carrying the real size. rotX/rotY/rotZ/translate
// COMPOSE INTO THE MATRIX — after generation there is no vertex work. Vertices
// are generated once per distinct `key` and every handle with that key shares
// the same mesh object, so a renderer can batch by key.
//
...
export function bake(g) {
  return { key: g.key, id: g.id, mesh: g.mesh, m: g.m.slice(), t: g.t.slice() };
}
...
// reference, so a renderer can draw one call per key
