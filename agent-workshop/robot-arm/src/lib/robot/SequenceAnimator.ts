...
export class SequenceAnimator {
    ...
    init(mjModel: MujocoModel, getName: (addr: number) => string) {
        this.trayId = -1;
        for (let i = 0; i < mjModel.nbody; i++) {
            if (getName(mjModel.name_bodyadr[i]) === 'stack_base') this.trayId = i;
        }
    }
    ...
    prepareStep(ikTarget: THREE.Object3D, mjData: MujocoData, ikSystem: IkSystem) {
         ...
         // 3x3 grid stack on top of stack_base
         ...
         dropPos.x += (row - 1) * 0.06;
         dropPos.y += (col - 1) * 0.06;
