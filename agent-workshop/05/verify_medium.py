"""Lesson 05 — loop + guardrails + rule-based verifier (medium)."""
...
def t_pickup():
    ...
    if not h: raise ValueError("not hovering over any cube; call move_to above a cube first")
...
def t_place(index=0):
    if not WORLD.get("holding"): raise ValueError("not holding any cube; pickup before place")
    trays = WORLD.get("trays") or []
    if not trays: raise ValueError("no trays available")
    t = trays[int(index)]
    x, y, z = t["pos"]
