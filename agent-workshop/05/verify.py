"""Lesson 05 — lesson 04 + verification tools + rule-based completion."""
...
def t_list_trays():
    r = requests.get(f"{ARM}/api/tray", timeout=10).json()
    WORLD["trays"] = [{"name": t["name"], "pos": t["pos"], "size": t["size"]}
                      for t in r.get("trays", [])]
...
def t_check_gripping():
    r = requests.get(f"{ARM}/api/gripping", timeout=10).json()
    g = r.get("gripping")
    WORLD["holding"] = g.get("name") if g else None
    WORLD["grip"] = r.get("gripper")

def t_check_tray():
    r = requests.get(f"{ARM}/api/tray", timeout=10).json()
    WORLD["tray_contents"] = {t["name"]: [c["name"] for c in t["cubes"]]
                              for t in r.get("trays", [])}

TOOLS = {
    "get_workspace":  (t_get_workspace,  {}),
    "list_cubes":     (t_list_cubes,     {}),
    "list_trays":     (t_list_trays,     {}),
    "set_grip":       (t_set_grip,       {"value": "int"}),
    "move_to":        (t_move_to,        {"x": "float", "y": "float", "z": "float"}),
    "get_ee_pos":     (t_get_ee_pos,     {}),
    "check_gripping": (t_check_gripping, {}),
    "check_tray":     (t_check_tray,     {}),
}

def task_complete():
    r = requests.get(f"{ARM}/api/tray", timeout=10).json()
    g = requests.get(f"{ARM}/api/gripping", timeout=10).json()
    return any(t.get("cubes") for t in r.get("trays", [])) and not g.get("gripping")

SYS = ("Drive a robot arm. Read WORLD for current state (null=unknown, fetch via tools).\n"
       "RECENT_ACTIONS shows your last calls. Loop ends automatically when task done.\n"
       "Verify after critical steps: check_gripping after closing, check_tray after release.\n"
       "Tools:\n"
